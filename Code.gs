  ...
  try {
    ...
    if (data.originalTimestamp) {
      ...
      for (var i = sheetData.length - 1; i >= 1; i--) {
        if (sheetData[i][0] === data.originalTimestamp &&
            sheetData[i][1] === data.consultantName &&
            sheetData[i][2] === data.schoolName) {
...
function getServerHistory(searchConsultant, searchSchool) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (consultant !== searchConsultant || school !== searchSchool) continue; 
    ...
    if (!historyMap[ts]) {
      historyMap[ts] = { timestamp: ts, consultantName: consultant, schoolName: school, items: [] };
    }
    historyMap[ts].items.push({ grade: grade, type: type, content: content, imageUrl: imgUrl, imageBase64: '' });
  }
...
// 5. [추가] PDF 생성을 위해 서버에서 직접 이미지 데이터를 추출하여 전달하는 함수
  ...
  for(var i = 0; i < urls.length; i++) {
    ...
    if(fileId) {
      try {
        ...
        map[url] = 'data:' + blob.getContentType() + ';base64,' + b64;
      } catch(e) {
        map[url] = '';
      }
    }
